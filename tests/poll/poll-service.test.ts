...
describe('pollService — health', () => {
  ...
  it('unhealthy poll synthesizes event and reaches Down at threshold', async () => {
    ...
    const first = await pollService(store, http, noopDeps, await currentPollable(), now)
    ...
    const second = await pollService(store, http, noopDeps, await currentPollable(), now)
  ...
  it('timeout/connection failure counts as unhealthy', async () => {
    ...
    const outcome = await pollService(store, http, noopDeps, await currentPollable(), now)
  ...
  it('healthy poll resets failures, resolves health issue, refreshes health', async () => {
    ...
    await pollService(store, bad.http, noopDeps, await currentPollable(), now)
    await pollService(store, bad.http, noopDeps, await currentPollable(), now)
    ...
    const outcome = await pollService(store, good.http, noopDeps, await currentPollable(), now)
...
describe('pollService — errors endpoint', () => {
  ...
  it('processes errors with externalId dedup and advances cursor', async () => {
    ...
    const outcome = await pollService(store, http, noopDeps, pollable(c), now)
    ...
    const again = await pollService(store, http, noopDeps, pollable(c), now)
  ...
  it('appends since=cursor to the request when cursor exists', async () => {
    ...
    await pollService(store, http, noopDeps, pollable(config({ healthUrl: null, errorUrl: 'https://a/errors', cursor: '2026-07-28T10:00:00.000Z' })), now)
  ...
  it('fetch failure flags errorFetchFailed and keeps cursor', async () => {
    ...
    const outcome = await pollService(store, http, noopDeps, pollable(config({ healthUrl: null, errorUrl: 'https://a/errors' })), now)
...
describe('pollService — health + errors combined', () => {
  it('error-branch cursor write preserves health-branch poll state', async () => {
    ...
    const outcome = await pollService(store, http, noopDeps, pollableNow, now)
...
describe('runPoll', () => {
  it('polls only due services', async () => {
    ...
    const outcomes = await runPoll(store, http, noopDeps, now)
