...
describe('handleIngest', () => {
  ...
  it('401 when any auth header is missing', async () => {
    ...
    for (const over of [{ serviceName: null }, { timestamp: null }, { signature: null }]) {
      const res = await handleIngest(store, noopDeps, request(body, over), now)
  ...
  it('401 for unknown service and service without secret (indistinguishable)', async () => {
    const body = '{"message":"x"}'
    const unknown = await handleIngest(store, noopDeps, request(body, { serviceName: 'nope' }), now)
  ...
  it('401 for bad signature and stale timestamp', async () => {
    const body = '{"message":"x"}'
    const bad = await handleIngest(store, noopDeps, request(body, { signature: 'sha256=' + '0'.repeat(64) }), now)
  ...
  it('400 for invalid JSON (signature valid over the raw bytes)', async () => {
    const body = 'not json'
    const res = await handleIngest(store, noopDeps, request(body), now)
  ...
  it('repeat events aggregate into the same issue', async () => {
    ...
    await handleIngest(store, noopDeps, request(body), now)
    const res = await handleIngest(store, noopDeps, request(body2), now)
    expect(res.status).toBe(201)
