...
服務 ──POST /api/ingest (HMAC 金鑰)──┐
                                     ├─▶ 正規化 ─▶ 依 fingerprint upsert issue
Sentry ──Cron 拉取 /api/poll/sentry ─┘        ├─▶ 規則引擎判級 (P0/P1/P2)
                                              ├─▶ Discord 通知器 (去重 / 冷卻聚合)
                                              └─▶ Supabase Realtime ─▶ Dashboard 即時更新
```

**核心原則**：ingest 與 Sentry 兩條入口匯流到相同的 `normalize → upsert → rules → notify` 管線。日後新增來源（如自建 log）只需再寫一個 normalizer，管線本體不動。
...
### 4.5 健康度計算
- 服務健康狀態由「近 N 分鐘內最嚴重的**未解**（status = open/acknowledged）issue」推導：
  - 有未解 P0 → 🔴 **Down**
  - 有未解 P1 → 🟡 **Degraded**
  - 否則 → 🟢 **Healthy**
- N 為可設定參數（預設值於實作計畫決定）。
- 已知限制：推送制無心跳，無法分辨「服務真的掛了」與「剛好沒有錯誤」。**未來擴充**：加入 heartbeat / last-seen 偵測靜默失聯（不在 MVP）。
...
- `services` — 服務清單、webhook secret、Sentry 對應、Discord 路由設定、健康度視窗設定。
