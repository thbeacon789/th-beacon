...
2. **Vercel env**：`NEXT_PUBLIC_SUPABASE_URL`（client/server 共用）／`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`（sb_publishable_...）／`SUPABASE_SECRET_KEY`（sb_secret_...）／`CRON_SECRET`／`DISCORD_WEBHOOK_URL`（選）／`APP_URL`（選）。金鑰用 hosted 專案的新制 API keys（非 legacy anon/service_role JWT）。
